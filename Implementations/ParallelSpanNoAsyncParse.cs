...
public class ParallelSpanNoAsyncParse : IProcessFile
{
    public ParallelSpanNoAsyncParse(int bufferSize = 10_000_000)
    {
    ...
    public ValueTask<Dictionary<string, Measurements>> ProcessFile(string filename)
    {
        ...
        using (var fileStream = new FileStream(filename, FileMode.Open))
        {
            ...
            List<Func<Dictionary<string,Measurements>>> processTasks = new List<Func<Dictionary<string,Measurements>>>();
            ...
            Parallel.ForEach(processTasks, task => 
            {
                results.Add(task());
            });
